...
struct Song {
    ...
    func songInformation2(title: String, artist: String, duration: Int) -> String {
        return "\(title) by \(artist), duration \(duration)s"
    }
}
//:- callout(Exercise): Below, use the `Song` struct from the previous page to simplify your code.
/* Create the array of songs here */
let songTitles2 = ["Ooh yeah", "Maybe", "No, no, no", "Makin' up your mind"]
let artists2 = ["Brenda and the Del-chords", "Brenda and the Del-chords", "Fizz", "Boom!"]
let durations2 = [90, 200, 150, 440]

/* Declare the songInformation function here */
let song = Song(title: "No, no, no", artist: "Fizz", duration: 150)
song.songInformation2(title: songTitles2[i], artist: artists2[i], duration: durations2[i])
...
for i in 0 ... songTitles.count - 1 {
    print(songInformation2(title: songTitles[i], artist: artists[i], duration: durations[i]))
}
