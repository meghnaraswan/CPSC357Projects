...
//        display.setPixel(x: x + i, y: y + length, color: color)
//    }
//}
//
...
//        display.setPixel(x: x + length, y: y + i, color: color)
//    }
//}

func hline(x: Int, y: Int, length: Int, height: Int, color: Color) {
    for i in 0 ... length - 1 {
        display.setPixel(x: x + i, y: y, color: color)
        display.setPixel(x: x + i, y: y + height - 1, color: color)
    }
}

func vline(x: Int, y: Int, length: Int, width: Int, color: Color) {
    for i in 0 ... length - 1 {
        display.setPixel(x: x, y: y + i, color: color)
        display.setPixel(x: x + width - 1, y: y + i, color: color)
    }
}

func block(x: Int, y: Int, width: Int, height: Int, color: Color) {
    hline(x: x, y: y, length: width, height: height, color: color)
    vline(x: x, y: y, length: height, width: width, color: color)
}

block(x: 2, y: 3, width: 20, height: 10, color: .cyan)
/*:
 ### Fill Speed
...
func Pixels(x: Int, y: Int, width: Int, height: Int, color: Color){
    for a in 0 ... width - 1 {
        for b in 0 ... height - 1 {
            print("\(a) \(b)")
            display.setPixel(x: a + x, y: b + y, color: color)
        }
    }
    
}

Pixels(x: 2, y: 17, width: 20, height: 10, color: .magenta)
/*:
